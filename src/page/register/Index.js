...
export function Index() {
    ...
    const handleDestroy = async () => {
        ...
        try {
            ...
            toast.success('Đã xóa đăng ký môn học thành công');
            getAPIData();
    ...
    // Fix lỗi khi click lại vào NavLink register đang active
    useEffect(() => {
    ...
    const getAPIData = async () => {
        try {
            let response = await axios.get(
                `${process.env.REACT_APP_API_URL}/api/v1/registers?page=${currentPage}&search=${search}`, {
                headers: {
                    'Authorization': `Bearer ${getAcessToken()}`
                }
            }
    ...
    return (
        <>
            ...
            <Helmet>
                <title>Danh sách đăng ký môn học | QLSV</title>
            </Helmet>
            ...
            <Search handleSearch={(e) => handleSearch(e)} search={search} />
            {
                !isLoaded ?
                    ...
                    :
                    <>
                        <RegisterList registers={registers} handleShowDialog={handleShowDialog} />
                        <div>
                            <span>Số lượng: {registers.length}/{totalItem}</span>
                        </div>
